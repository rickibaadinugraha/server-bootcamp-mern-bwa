...
const signupParticipant = async (req) => {
  ...
  // if email and status inactive run
  ...
  if (result) {
    ...
    result.role = role;
    result.otp = Math.floor(Math.random() * 9999); // random only 4 number
    await result.save();
  } else {
...
const activateParticipant = async (req) => {
  ...
  if (!check) throw new NotFoundError("Participant not yet registered");
...
const signinParticipant = async (req) => {
  ...
  if (!result) throw new UnauthorizedError("Invalid credential error");
  if (result.status === "Inactive")
    throw new UnauthorizedError("Your Account must be active");

  const isPasswordCorrect = await result.comparePassword(password);
  if (!isPasswordCorrect)
    throw new UnauthorizedError("Invalid credential your password");
...
// Task create email send if CO
const checkOutOrder = async (req) => {
  const { event, personalDetail, payment, tickets } = req.body;

  const checkingEvent = await Events.findOne({ _id: event });
  if (!checkingEvent) {
    throw new NotFoundError("Not found event with id: ", +event);
  }
