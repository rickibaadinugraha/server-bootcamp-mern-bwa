...
const createPayments = async (req) => {
  ...
  if (check) {
    throw new BadRequestError("Duplicate payment method");
  }
...
const getOnePayments = async (req) => {
  ...
  if (!result) {
    throw new NotFoundError(`Not dfoun payment method with: ${id}`);
  }
...
const updatePayments = async (req) => {
  ...
  if (!check) {
    throw new NotFoundError("Duplicate payment type");
  }
  ...
  if (!result) {
    throw new NotFoundError(`Not found speakers with id: ${id}`);
  }
...
const deletePayments = async (req) => {
  ...
  if (!result) {
    throw new NotFoundError(`Not found payment type with id: ${id}`);
  }
